...
这不是一个单纯查看微信读书数据的工具，也不是一个泛用 AI 聊天壳。
...
本项目的数据访问基于微信读书官方提供的 Skills / API Key 体系，
**不破解、不逆向、不绕过平台限制**——你的凭证和 Key 始终在你本地。

官方 Skills 擅长的是：让对话式 AI 即时查你的阅读信息。
本项目的定位是把它进一步变成 **本地资产**：
- 离线可用的书架与笔记浏览
- 批量导出（而非逐条聊天复制）
- 可配置的 AI 复盘 Skill（模板化、可复用的本地产物）
- 你的数据你留底（Local-first）
