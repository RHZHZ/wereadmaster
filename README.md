...
- **阅读状态更准确**：候选关系、阅读生命周期和整理状态拆分为三个独立维度；加入候选、开始阅读、待整理和已整理不再互相覆盖。首次升级会先创建本地数据库备份，并保留上一正式版可读取的兼容字段。
- **导出流程统一**：单项成果统一支持 Markdown、Obsidian、Notion 独立多选；书籍复盘支持多本书 × 多目标批量导出、独立执行页、目标级结果和失败项精确重试。
- **Notion 更易接入**：可以连接用户已有数据库，也可以创建带推荐视图的标准阅读成果库；标签会随页面创建请求自动写入，新标签若被 Notion 拒绝会明确标记对应目标失败，不再静默跳过。
- **边界更清楚**：Web Preview 保持只读，导出不会自动请求微信读书或 AI；设置页明确区分 SQLite 备份、凭据和本地阅读器浏览器存储的覆盖范围。
- **体验与兼容性收口**：完善短视口、触摸设备和批量导出滚动布局，统一导出语义与结果反馈，并同步 Windows x64、Android APK 和 Web 只读预览的数据契约。

详细升级、兼容与回退说明见 [v1.0.17 Release Notes](docs/release-notes-v1.0.17.json)。
...
- 把阅读直接连接到 Markdown、Obsidian、Notion、写作整理和知识沉淀流程。
...
- **从阅读到输出，闭环完整**：书架、笔记、统计、复盘、行动项和 Markdown / Obsidian / Notion 导出连成一条线。
...
- **AI 阅读助手**：在全局、当前书和候选书架场景中用本地阅读上下文对话，支持流式输出、上下文开关、对话历史和本地清空；新书推荐可先加入本地候选，再通过微信读书搜索确认书源。
- **AI Provider 兼容配置**：设置页支持 OpenAI、DeepSeek、通义千问、Kimi 和自定义 Provider 预设；可测试连通性、测试结构化输出兼容性，也可刷新可用模型并保留手动输入兜底。
- **知道下一步该做什么**：通过阅读指南、跨书路线和选书决策，帮助判断下一本读什么、这本怎么推进、何时值得复盘，并把行动反馈沉淀为后续更新依据。
- **从公共线索补全判断**：书籍详情可查看热门划线、公开书评和划线下读后感，帮助在阅读前后快速补充外部参考。
- **把成果带走**：单项成果支持 Markdown、Obsidian、Notion 独立多选；笔记和书籍复盘提供批量导出、目标级结果与失败项重试，报告图片继续使用独立生成、保存和分享流程。

## AI 阅读资产

应用把 AI 能力收束成可持续保存的阅读资产与受控对话，不做开放式闲聊，也不会在后台自动上传内容。
...
| 阅读指南 | 回答“这本书接下来怎么读、怎么整理、怎么复盘”。 | 在书籍详情点击“本书阅读指南”，默认只基于当前书、本地进度、已有复盘和统计信号生成。路线图节点可点开核对完整目的、依据和投入。 | 指南会按书归档到“复盘 > 阅读指南”；下一步行动清单是唯一反馈入口，节点只展示只读关联行动，适合在阅读推进、笔记增加或读完后刷新。 |
| 跨书指南 | 回答“围绕当前主题，下一步读哪些书、按什么顺序读”。 | 先把候选书加入候选书架，再在阅读指南里勾选候选书生成跨书路线图。 | 跨书路线会和当前书关联保存，后续可回到阅读指南库查看路线、复盘节点和下一步动作；行动反馈会作为阅读成果保留，不和说明性节点混在一起。 |
| AI 阅读助手 | 回答临时阅读问题、整理本地上下文、推荐可加入候选书架的新书。 | 点击右下角 AI 阅读助手，在全局、当前书或候选书架范围内提问；可开关上下文，推荐书可直接加入候选或先搜索确认。 | 对话历史保存在本地，可按线程查看、删除或清空；推荐候选会标记来源和微信读书确认状态。 |

持续记录的推荐节奏：

1. 同步书架和笔记后，先在书籍详情确认当前书状态。
2. 阅读中期生成或刷新“本书阅读指南”，明确下一段阅读范围和复盘输出，并在下一步行动清单记录执行反馈。
...
5. 通过 Markdown、Obsidian、Notion、批量导出和报告图片，把成果带到你的长期知识系统里。

## 页面预览

| 总览 | 书架 | 笔记 | 设置 |
| --- | --- | --- | --- |
| ![](src/assets/hero-reading-dashboard.jpg) | ![](src/assets/shelf.png) | ![](src/assets/notes.png) | ![](src/assets/generated/set.png) |

## 快速开始

当前正式发布面向 Windows x64，并随 GitHub Release 提供 Android 签名 APK。

1. 从 [GitHub Releases](https://github.com/RHZHZ/wereadmaster/releases) 下载 Windows 安装包；Android 用户下载签名 APK。
2. 安装并启动应用。
3. 在设置页完成凭据和更新配置；如果移动端本地凭据存储不可用，设置页会显示可排查的错误说明。
4. Android 版会隐藏桌面窗口控制，使用移动端顶部栏和侧边抽屉导航。
5. 如果 Android 上已配置代理但微信读书仍无法同步，可在“设置 > 账户与同步 > 微信读书网络代理”填写代理工具提供的 HTTP 或 SOCKS 地址，例如 `http://127.0.0.1:7890` 或 `socks5://127.0.0.1:1080`。该配置只作用于微信读书同步接口，不影响 AI Provider 或应用更新。
6. 如需 AI 复盘或本地阅读器 AI 提问，在“AI 设置”中选择 Provider 预设、填写 Key，并测试连通性与兼容性。
7. 开始同步、导入本地图书、查看和整理你的阅读数据。
...
- AI 阅读助手支持流式对话、上下文范围控制、历史线程管理、新书推荐和候选书源确认。
- 本地阅读器支持 EPUB、TXT、Markdown 阅读，支持选区划线、想法、AI 提问和 Markdown 导出。
- 书籍详情支持热门划线、公开书评和划线下读后感面板，并保留 Skill 升级类错误的专门提示。
...
- 支持 Markdown、Obsidian、Notion 单项多目标导出；支持笔记和书籍复盘批量导出、目标级结果与失败项重试；移动端阅读报告图片可直接保存到系统相册或通过系统分享。
...
- 如因平台规则变化导致接口不可用，本项目以“本地已有缓存/已导出文件”作为底线保障（Local-first）。
