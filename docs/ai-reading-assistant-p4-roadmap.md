...
结论：P4 已关闭，可以进入 P5 或后续独立小需求阶段。

关闭依据：

- P4.1-P4.5 均已满足完成标准。
- 普通问答和新书推荐已走聊天流式入口，最终仍保持结构化输出合同。
- 取消、断流、Provider 429/500、非法 SSE、网络错误均已验证输入区恢复，不留下半截成功历史。
- 长回答、复杂推荐卡片、超量快捷追问和 80 条消息压力场景均未出现横向溢出或布局阻断。
- 阅读记忆、原始笔记和敏感字段边界已通过 Rust 单测和桌面联调验证。
- 真实 Provider 小样本已验证普通问答和新书推荐卡片渲染。
- 后续聊天控制补强已完成端到端自动化覆盖：
  - 分类书单可点击打开现有书籍详情。
  - 最后一条用户消息可编辑并重新生成。
  - 历史支持搜索、场景筛选和打开历史对话。
  - 模型控件已下沉到输入框，生成中菜单动作禁用。
  - 普通问答和新书推荐均覆盖流式增量展示。
  - 生成中取消路径覆盖 `cancel_reading_assistant_stream`。
...
- TypeScript：通过。
- 构建：通过。
- 单元测试：72 个测试文件，402 个测试通过。
- E2E：84 个测试通过。
- `git diff --check`：通过，仅有 Windows 工作区 LF/CRLF 提示。

保留但不阻塞 P4 的事项：

- 50 个历史线程压力场景未在用户当前真实库内批量制造；该项应放到临时数据目录或专用测试库中验证。
- 真实超时未单独等待长时间复现；未监听端口网络错误已覆盖“失败后输入区恢复”主链路。
- 聊天框内快速切换已拉取模型仍为后续可选增强，不属于 P4 完成条件。
- 手动对话分组、任意消息编辑、多分支对话树仍不建议进入近期范围。

## P5 输入清单

P5 不应继续扩大 AI 阅读助手产品面。建议拆成两类：

独立执行计划见：`docs/ai-reading-assistant-p5-stability-plan.md`。

### P5-A：稳定性与工程整理

- 在临时数据目录或专用测试库中补 50 个历史线程压力验证。
- 将真实 WebView2/CDP 验收脚本沉淀为可重复 QA 脚本，但不默认调用真实 Provider。
- 视需要拆分体量过大的测试 mock，降低 `tests/e2e/app-smoke.spec.ts` 维护成本。
- 仅在 P4/P5 验证稳定后再评估 `reading-api.ts` 或 AI 服务层拆分。

### P5-B：可选产品小步

- 当前 Provider 下快速选择已拉取模型；仍不编辑 API Key 和 Base URL。
- 当前书籍上下文历史的“只看当前对象”轻量筛选。
- 推荐书卡片的搜索确认和加入候选路径继续细化。
- 对跨书笔记语义检索另行做需求分析；当前结构化事实查询不引入向量数据库。
