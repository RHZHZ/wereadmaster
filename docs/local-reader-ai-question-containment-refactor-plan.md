...
当前本地阅读器已经支持用户选中文本后向 AI 提问，并且请求边界已经收口为“只发送选区和问题”。但展示层如果把 AI 提问默认渲染在每个章节、分区或正文段落下，会带来几个问题：
...
```text
AI 提问只在用户主动选区后出现入口，回答结果默认收纳为侧栏摘要卡，完整内容通过详情弹窗查看，不干扰正文阅读，不撑大卡片，不扩大 AI 数据输入范围。
```
...
应对：提问面板和详情中明确显示“基于选区”，并在请求层继续禁止整书输入。
