# WeReadMaster v1.0.11 微信公众号推广文案

> 推荐标题：我把微信读书做成了一个本地阅读工作台：书架、笔记、统计和 AI 助手都能沉淀下来
>
> 备用标题 1：读过的书，别只剩进度条：这个工具能把微信读书变成你的个人阅读资产库
>
> 备用标题 2：微信读书笔记太散？新版 WeReadMaster 帮你本地保存、复盘和规划下一本书
>
> 备用标题 3：WeReadMaster v1.0.11：AI 阅读助手上线，让阅读复盘更有上下文
>
...
这次发布的是 **v1.0.11**。新版本最核心的变化是：AI 阅读助手不再只是“问一句答一句”，而是开始接入你的阅读场景；候选书、热门划线、公开书评这些线索，也能更自然地进入阅读判断和复盘流程。
...
新版里的 AI 阅读助手不只支持一个通用聊天入口，而是会跟随你当前所在的阅读场景：

1. 全局范围：回看整体阅读状态。
2. 当前书范围：围绕一本书提问、整理和复盘。
3. 单本笔记范围：基于已有划线和想法追问。
4. 统计范围：解释阅读节奏、分类偏好和长期投入方向。
5. 候选书架范围：讨论下一本书该怎么选。

![真实桌面端书架：数据已同步到本机工作台](assets/wechat-v1.0.11/desktop-wechat-shelf.png)

## AI 阅读助手：不是陪聊，而是阅读助理

新版增加了右下角 AI 阅读助手入口。

它支持流式输出、取消生成、上下文开关、线程历史、最后一条用户消息编辑、历史搜索和当前对象筛选。模型状态也收进了输入框里的紧凑入口，不再额外占用聊天区。你可以打开或关闭上下文，也可以看到回答依据的大致范围，避免 AI 把一个简单问题扩展成不可控的大段内容。
...
这里的重点不是“AI 很聪明”，而是它能被限定在阅读任务里工作。

## AI 推荐新书后，先确认书源再沉淀

v1.0.11 补了一条很关键的工作流：**AI 推荐新书后，可以先进入本地候选书架，再通过微信读书搜索确认书源**。
...
- 新增 AI 阅读助手，支持全局、当前书、单本笔记、统计、候选书架等阅读上下文。
- 支持流式输出、取消生成、上下文开关、线程历史、最后一条用户消息编辑、历史搜索和当前对象筛选。
- 模型状态下沉到输入框区域，生成中会禁用模型菜单动作，避免状态错乱。
- AI 新书推荐可加入本地候选，也可通过微信读书搜索确认书源。
- 阅读助手可按分类列出本地可验证书目，不把统计总数扩写成无法确认的书名。
- 候选书架支持来源识别和筛选。
- 书籍详情新增热门划线、公开书评和划线下读后感。
- 总览、书籍详情、候选书架、AI 助手和空状态的信息密度更清晰。
- Windows 桌面端和 Android APK 持续通过 GitHub Releases 分发。
