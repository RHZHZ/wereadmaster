...
分类书目查询之外，按具体书名、标题前缀或作者确认阅读记录属于另一类需求。例如本地笔记书库已经能找到“富爸爸”相关书籍，但用户问“我读过富爸爸的书吗”时不能只依赖分类路由。该扩展见 [AI 阅读助手书名、标题前缀与作者阅读记录查询改造设计](./ai-reading-assistant-book-title-series-query-refactor-design.md)。
