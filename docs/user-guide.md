...
| 整理一本书的阅读成果 | `书籍详情 > 书籍复盘` 或 `复盘` | 需要已同步的笔记或本地缓存 |
| 制定下一步阅读顺序 | `书籍详情 > 本书阅读指南` | 可从单本扩展到候选书路线 |
| 查看长期阅读趋势 | `统计` | 总计、年度、月度、周度均可下钻 |
| 导出阅读成果 | `笔记`、`复盘`、`本地阅读器` | 单项成果可选 Markdown、Obsidian、Notion；批量流程按页面提供 |
| 备份或迁移本机数据 | `设置 > 高级维护` | 操作前建议先导出备份 |
| Android 同步排障 | `我的 > 微信读书代理设置` 或 `设置 > 账户与同步` | 只影响微信读书同步网络 |
...
2. 在卡片视图和章节视图之间切换。
3. 需要整理时，先看“建议复盘”。
4. 可以导出 Markdown，或直接进入 书籍复盘。
5. 如果你只是想先看材料密度，笔记页的统计条就足够判断这本书值不值得整理。

### 单项文档导出

单本笔记、书籍复盘、周期复盘、阅读指南、阅读路线和选书决策使用一致的文档导出流程：

1. 在当前成果页点击语义明确的 `导出…` 按钮。
2. 独立勾选 Markdown、Obsidian、Notion 中至少一个已经配置的目标。
3. 核对只读目的地摘要后开始导出。弹窗不会修改设置页里的长期配置。
4. 在结果页逐目标查看成功、失败、跳过、警告、路径或 URL；部分失败时只重试失败目标。

报告图片和笔记卡片图片仍使用各自的生成、下载、保存或分享操作，不进入文档导出弹窗。
...
1. 在 `复盘 > 书籍复盘` 打开 `导出书籍复盘`。
2. 在选择页搜索、筛选并勾选已经生成复盘的书；未生成的书不会因导出而自动调用 AI。
3. 在设置页独立选择 Markdown、Obsidian、Notion，可同时选择多个目标；目的地只读使用设置页已经保存的目录、Vault 和 Notion 数据库。
4. 点击 `开始导出` 后会切换到独立执行页。执行期间关闭按钮、背景点击和 Escape 都被禁用，因为底层任务没有取消能力。
5. 完成后按每个“书籍 × 目标”查看成功、失败、跳过、警告、文件路径或 Notion URL。
6. 部分失败时使用 `重试失败项`，只重新提交精确失败的书籍与目标，不重复导出已成功项。

这两个批量流程都只使用已有本地缓存或已生成资产。打开向导、预检或执行导出不会在后台自动同步微信读书，也不会自动调用 AI。
...
- 按周期查看已经生成的阅读成果。
- 可回看、更新，并导出到已配置的 Markdown、Obsidian 或 Notion 目标。
- 如果统计数据变化了，可以重新生成，旧版本会留在历史里。

## 9. 书籍详情

1. 先看书籍信息和本地整理状态。
2. 可标记 `待整理`、`已整理`、`已读完` 等状态。
3. 可进入：
   - `书籍复盘`
   - `本书阅读指南`
   - `找相似`
4. 还能看热门划线、公开书评和划线下读后感。
5. 如果你还没决定要不要整理，先把状态标成 `待整理` 通常最稳。
...
- 修改 Markdown、复盘、诊断等本地文件的导出目录。
- 配置 Obsidian Vault、附件策略和导出后打开方式。
- 配置 Notion Integration、连接用户已有数据库，或在共享父页面下创建标准阅读成果库。
- 单项和批量导出窗口只读使用这些长期设置，不会在窗口内临时改写目的地。
- 只会影响之后新生成的文件或页面，不会搬动旧文件，也不会迁移已有 Notion 页面。
...
- SQLite 备份包含主数据库及存在时的 WAL/SHM 文件，不包含微信读书、AI、Notion API Key 或安全存储文件。
- 本地阅读器划线、想法、AI 提问记录、阅读器偏好和人工版本关联位于浏览器/WebView 存储，v1.0.17 暂不包含在 SQLite 备份包内；重要内容请另行导出。
...
- 微信读书同步失败时，优先在 `我的 > 微信读书代理设置` 或 `设置 > 账户与同步` 检查代理。代理地址应使用代理工具实际提供的 HTTP 或 SOCKS 端口。
...
| 产物 | 入口 | 内容边界 |
| --- | --- | --- |
| 单本笔记文档 | `笔记 > 某本书` | 可导出到 Markdown、Obsidian、Notion；包含划线、想法/点评和章节信息，不含书签正文 |
| 批量笔记文档 | `笔记 > 批量导出` | 按预检和筛选结果导出多本书的本地缓存 |
| 单本书籍复盘文档 | `书籍详情 > 书籍复盘` 或 `复盘` | 可导出到 Markdown、Obsidian、Notion；只使用已生成的本地复盘，不会因导出自动请求 AI |
| 批量书籍复盘文档 | `复盘 > 书籍复盘 > 导出书籍复盘` | 多本书 × 多目标执行，保留目标级结果并支持精确失败项重试 |
| 阅读指南 / 阅读路线文档 | `复盘 > 阅读指南` | 已生成的单本阅读指南或多本阅读路线，可选择多个文档目标 |
| 周期复盘 / 选书决策文档 | 对应成果页 | 导出当前已经生成的资产，不在导出时重新生成 |
| 报告图片 PNG | `统计` 或 `成果 > 周期复盘` | 周报、月报、年报或总计周期复盘的图片 |
| 本地阅读器 Markdown | `本地阅读器` | 本地书的划线、标记、想法和本地 AI 提问记录 |
| 诊断信息 | `设置 > 高级维护` | 用于排障的环境和状态信息，不应包含 API Key |
| 本地备份包 | `设置 > 高级维护` | 本地数据库和辅助文件，不包含 API Key |

导出目录、Obsidian Vault 和 Notion 数据库可在 `设置 > 导出设置` 统一管理。文件名和具体子目录由应用生成；修改配置不会搬动历史文件或迁移已有 Notion 页面，建议每次导出后根据目标级成功结果记录实际路径或 URL。
...
3. 先开 `本书阅读指南`，确认下一步读法。
4. 读完后进 `书籍复盘`。
5. 最后选择 Markdown、Obsidian 或 Notion 导出目标。
...
3. 进入 `复盘` 整理成结构化结果。
4. 再去 `统计` 看长期趋势。
5. 需要对外分享时导出报告图片，或把文档写入 Markdown、Obsidian、Notion。
...
4. 如果错误提示明确要求升级 Skill 或接口版本，按提示升级后再重试；反复失败时先使用本地已有缓存或已导出文件。
...
v1.0.17 会把清理并去重后的标签随页面创建请求提交，不再因为数据库选项中暂时没有同名标签就静默跳过。应用不会额外 PATCH 用户数据库 schema；如果 Notion 拒绝某个标签或属性，对应目标会明确失败。请在结果页查看错误，确认 Integration 对数据库有写入权限、`标签` 映射仍指向 Multi-select 属性，必要时在 Notion 中手工补充选项后使用 `重试失败项`。
...
- API Key 和 AI API Key 由用户自行配置，按应用设计只在本机使用，不写入 Markdown / Obsidian / Notion 导出和诊断信息。
